          ...
          <div className={styles.progressRow}>
            <div className={styles.progressTrack}>
              <div
                ...
                style={{ width: `${Math.round(((answeredCount ?? 0) / (participants ?? 1)) * 100)}%` }}
              />
            </div>
      ...
      <div className={styles.revealWrap}>
        ...
        <div className={styles.barList}>
          {options.map((opt, i) => {
            ...
            const pct = Math.round(total > 0 ? (count / total) * 100 : 0);
            const barWidth = barsReady ? `${(count / maxCount) * 100}%` : "0%";
                  ...
                  <div key={s.questionIndex} className={styles.summaryItem}>
                    ...
                    <div className={styles.summaryMiniBar}>
                      {s.options.map((opt, i) => {
                        ...
                        const count = counts[i] ?? 0;
                        const pct = Math.round(totalAns > 0 ? (count / totalAns) * 100 : 0);
                        const fillW = barsReady ? `${(count / maxC) * 100}%` : "0%";
                        return (
