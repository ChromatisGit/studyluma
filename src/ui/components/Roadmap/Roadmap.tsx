...
function RoadmapTopic({
  ...
  return (
    <div className={clsx(styles.topicItem, !isLast && styles.topicItemSpaced)}>
      {/* Segment line - extends full height of this topic item */}
      {showSegment && (
        <div
          className={clsx(styles.segment, segmentIsFinished ? styles.segmentFinished : styles.segmentPlanned)}
        />
      )}
      ...
      {chapters.length > 0 && (
        <div
          className={clsx(styles.chaptersContent, isExpanded && styles.chaptersContentExpanded)}
          style={{ paddingLeft: showIcons ? "42px" : 0 }}
        >
          <div id={panelId} role="region" aria-label={UI_TEXT.roadmap.chaptersAriaLabel.replace("{label}", label)}>
...
function RoadmapBullet({ status }: RoadmapBulletProps) {
  ...
  return (
    <div className={clsx(styles.bullet, bulletClass)}>
      {status === "finished" && (
...
function RoadmapChapter({
  ...
  if (isSelectionMode) {
    return (
      <button
        ...
        className={clsx(styles.chapterLabel, styles.chapterSelectable, statusClass, isSelected && styles.chapterSelected)}
      >
        {label}
      </button>
    );
  ...
  if (isClickable) {
    return (
      <AppLink href={href} className={clsx(styles.chapterLabel, statusClass)}>
        {label}
      </AppLink>
    );
  }
  ...
  return (
    <span className={clsx(styles.chapterLabel, statusClass)}>
      {label}
    </span>
  );
}
